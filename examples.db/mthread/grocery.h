...
This is a class used to test the Persistent base class.
...
public: // Advanced database functions
  gxDatabaseError ConnectDB(POD *DB) { return connect_to_database(DB); }
  int ChangeObject(const Grocery &ob);
  int WriteName(const char *n, int find_object = 1, int flush_writes = 1);
...
private: // Internal Processing functions
  gxDatabaseError connect_to_database(POD *DB) { return Connect(DB); }
