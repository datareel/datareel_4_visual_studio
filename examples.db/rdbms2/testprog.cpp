...
program demonstrates how to create tables using gxRDBMS definition 
files.
*/
// ----------------------------------------------------------- // 
#include "gxdlcode.h"
...
int main(int argv, char **argc)
{
  ...
  // Set a default table definition for demonstration purposes
  gxString fname = "testdb1.def";

  if(argv < 2) {
    cout << "No DEF file specified on the command line" << "\n" << flush;
    cout << "Defaulting to " << fname.c_str() << "\n" << flush;
  }
  else {
    fname = argc[1];
  }
  ...
  if(!gxrdReadDefFile(fname, process_name, table_def, key_def,
		      family_def, fkey_def, user_def, group_def,
		      error_message)) {
    cout << error_message;
    return 1;
  }
